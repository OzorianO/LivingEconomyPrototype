...
M0, M1a (інвентар) та M1b (економіка героя) завершено; M1c у роботі:
[позиція героя та базова first-person камера](Docs/M1_HERO_POSE.md) реалізовані,
[смерть/loot і призупинений бізнес](Docs/M1_DEATH_LOOT.md) також реалізовані.
Нова standalone process-restart перевірка ще попереду; M1c/M1 не DONE.
...
Права кнопка + перетягування — камера; колесо — наближення у третій особі.
V — перемикання першої/третьої особи в режимі героя.
...
Death / loot demo — окремий сценарій із тілом NPC-01 біля його дому.
Підійти й натиснути E: можна забрати залишкові особисті предмети та монети.
Бойова система, respawn і спадкування ще не реалізовані.
...
Save/Load зберігає завершений економічний день у локальному XML-слоті v5
разом із гаманцем, предметами, потребами, позицією та камерою героя;
читач підтримує v1/v2/v3/v4/v5/v6. За наявності смерті XML v6 зберігає
стан/позицію тіл і їхнє залишкове особисте майно.
Перезавантаження коду відновлює завершений день, відкочуючи незавершений;
небезпечна збережена позиція дає fallback на spawn без втрати майна.
Ціни й продуктивність фіксовані,
...
18.09.2026: PASS, 31 201 твердження ядра, включно з 100-денними продовженнями,
збереженням грошей, атомарністю, порядком подій, наймом, звітами та XML.
Додатково PASS, 215 перевірок у Unity: реальне перезавантаження коду, карта,
камери, герой, ходьба, стрибки, стіни, взаємодії, недоступні маршрути, повторний Play та Load,
кольори посад і робочі позиції окремих підприємств, купівля/їжа, pose Save/Load,
first-person, безпечне відновлення героя, corpse/loot та заборона руху після смерті.
[Результат Unity](Docs/M1C_DEATH_UI_RELOAD_RESULT.txt).
